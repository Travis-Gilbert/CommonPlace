...
| d10-console-host-boundary | decision | agent | Resolve ownership, placement, and transport for the browser IDE before the console host consumes it. | done (2026-08-11; CommonPlace owns UI source and `/IDE`; Theorem retains protocol/backend) |
| console-host | work | agent | CommonPlace-owned browser IDE source and immutable wasm artifact served behind the existing authenticated `/IDE` edge, using the Theorem IDE protocol/backend seam. | parked (depends on ide-proxy-fold; apps/console shared scope remains dirty) |
| v-console-host | verify | agent | Verify ownership, artifact identity, same-origin transport, local browser behavior, and signed-in `v2.theoremharness.com/IDE` acceptance. | pending |
...
| ide-proxy-fold | work | agent | theorem ide-proxy subcommand: CLI's store instance → AgentFsHost/HostFs → serve_ws; one store handle. Depends on ws-merge. | occupied (2026-08-15: O-F.3 green, O-F.2 seam landed; --help/smoke wait on binary link) |
