...
| d10-console-host-boundary | decision | agent | Resolve ownership, placement, and transport for the browser IDE before the console host consumes it. | done (2026-08-11; CommonPlace owns console `/IDE` rollback). Amended 2026-08-16 by d12: AgentFS OS host is Theorem Dioxus, not this door. |
| console-host | work | agent | CommonPlace-owned browser IDE source and immutable wasm artifact served behind the existing authenticated `/IDE` edge, using the Theorem IDE protocol/backend seam. | parked-superseded (d12 2026-08-16: AgentFS stays in Theorem with the Dioxus site) |
| v-console-host | verify | agent | Verify ownership, artifact identity, same-origin transport, local browser behavior, and signed-in `v2.theoremharness.com/IDE` acceptance. | parked-superseded (d12 2026-08-16) |
...
| d11-agentfs-os | decision | agent | AgentFS POSIX + FUSE is the RustyRed OS (Fly same-kernel first-class); HostFs / unmounted MCP / git checkout as read path are fallback. | done (2026-08-16) |
| d12-agentfs-dioxus-host | decision | agent | AgentFS stays in Theorem and deploys with the Dioxus Rust site (`UI/`); CommonPlace `/IDE` is not the OS door. | done (2026-08-16) |
| dioxus-agentfs-host | work | agent | Deploy canvas-gallery on Fly on the same kernel as AgentFS FUSE + own volume. | done (2026-08-16; live https://travis-theorem-dioxus.fly.dev) |
| v-dioxus-agentfs-host | verify | agent | Live HTTP gallery + `/readyz` live_mount; own volume; same-kernel files. | done (GATE PASSED 2026-08-16) |
...
| ws-integration | work | agent | REFINED into ws-merge + ide-proxy-fold + g0-verify (+ v-ws-integration). Fork crates merge into rustyredcore_THG as theorem-ide-app/rpc/proxy/core; IDE proxy folds into theorem binary; g0 carry-forward verify items. | sealed (2026-08-16; children ws-merge + g0-verify + ide-proxy-fold done) |
...
start -> s0 (handoff: HANDOFF-LAPCE-FORK-SPIKE-1.0; board amendments 1-5) -> v-s0 -> g0-island-probe (rescoped acceptance) -> p-l3; s0 -> d7 -> d7m -> v-d7m; s0 -> d8; d8 -> d11-agentfs-os -> d12-agentfs-dioxus-host -> dioxus-agentfs-host -> v-dioxus-agentfs-host; s0 -> theorem-proxy -> v-theorem-proxy; s0 -> token-kernel -> v-token-kernel; (theorem-proxy + d7m + token-kernel + ws-transport) -> ws-integration; g0-verify -> d9-gl-fallback -> terminal (decision sealed 2026-08-10: WebGPU required at current floem pin); ws-integration -> wave-6 chart (p-l3-exec, token-kernel-binding, console-host).
