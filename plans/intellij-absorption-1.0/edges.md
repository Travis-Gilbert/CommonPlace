...
## d8 -> d11-agentfs-os -> d12-agentfs-dioxus-host -> dioxus-agentfs-host -> v-dioxus-agentfs-host (done 2026-08-16)
Handoff: live origin `https://travis-theorem-dioxus.fly.dev`. Same-kernel AgentFS FUSE at `/workspace` (`rustyred-agentfs` in `/proc/mounts`) + Dioxus canvas-gallery nginx on machine `8d067dbed54478`. Volume `dioxus_agentfs` (`vol_rnz6qz1wk5zg21er`) → `/data`. Runtime Debian `nginx:stable-bookworm` + glibc `theorem-workspace`. CommonPlace `/IDE` is not this door; do not occupy `console-host`. Remains: `ide-proxy-fold` still parked on wasmtime E0310 (`theorem ide-proxy --help` / Initialize→ReadDir). Optional later: theoremweb.com cert/DNS (fly.dev is the first receipt). Learned: alpine nginx cannot run the glibc workspace binary; do not `exec nginx` or the FUSE trap dies.
