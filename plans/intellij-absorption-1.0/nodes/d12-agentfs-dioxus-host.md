...
- **d10 amended, not erased:** CommonPlace still owns the console (`apps.theoremweb.com` / `v2.theoremharness.com`). `/IDE` there stays the existing Studio/code-server rollback. It is not the AgentFS OS door. `console-host` must not re-home the AgentFS workbench.
