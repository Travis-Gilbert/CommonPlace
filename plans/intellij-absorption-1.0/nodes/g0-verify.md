...
- O-G.3 DEFERRED-with-protocol: keystroke budget — no bundle (disk: needs ~3-4Gi transient); protocol + staged `keystroke-budget.js` (keydown → WebSocket.send pairing in one clock domain, 16ms pump accounted; expected median 8-16ms; median ≫32ms = budget violated); close-out commands in G0-VERIFY.md.
- O-G.4 PARTIAL: wgpu 24.0.5 `webgl` feature survival EXECUTED (verified in merged Cargo.toml + compiled into the wasm unit graph by the green wasm32 check); render attempt DEFERRED-with-protocol (staged `gl-fallback.js`: --disable-features=WebGPU, navigator.gpu probe, pixel probe; 3-step deferral ladder).
...
STATE: done. Evidence: `Theorem/docs/plans/intellij-absorption/G0-VERIFY.md`. Verify: parent v-ws-integration (pending ide-proxy-fold).
