...
- The canvas ground is a substantially faded, tokenized pegboard dot field. It replaces the installed demo starfield without changing the source component's spherical card placement, camera, or controls.
...
- The R3F scene renders on demand, with Drei controls invalidating frames during orbit and zoom. The pegboard is CSS, so Indexer has no second WebGL renderer or ambient background animation.
...
- Visual correction after canvas review: the Harvest summary overlay is removed. Source cards are larger, idle relationships remain visible but subdued, and the starfield is replaced by a substantially faded pegboard dot ground.
