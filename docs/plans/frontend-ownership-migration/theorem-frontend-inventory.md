...
| `design-check` self-contained HTML report | CLI/MCP report artifact, not a hosted app | `RETAIN-CONTRACT` | Keep report generation in Theorem; create and register `apps/web/src/components/commonplace/views/DesignScoutReportView.tsx` when FO-011 assigns the report descriptor |
