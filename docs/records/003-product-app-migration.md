...
- `crates/commonplace` holds the graph-native CommonPlace object model, ingest,
  organize, renderable-object, and block/view backend contracts.
...
`crates/commonplace` is now CommonPlace-owned product backend code. It still
depends on the sibling Theorem checkout for `rustyred-thg-core`, and
`apps/commonplace-api` / `crates/commonplace-desktop-runtime` still depend on
remaining sibling Theorem substrate and harness crates via local paths. Those
should become published crates, git-pinned crates, or CommonPlace-owned adapter
crates before CI or remote desktop packaging treats the native runtime as fully
independent of a local Theorem checkout.
