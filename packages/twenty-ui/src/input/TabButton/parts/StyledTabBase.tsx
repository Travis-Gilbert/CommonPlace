...
export const StyledTabButton = forwardRef<HTMLElement, StyledTabButtonProps>(
  ({ active, disabled, to, className, children, ...rest }, ref) => {
    // Replaces the legacy Linaria `as` polymorphism: LinkAdapter (next/link)
    // when a `to` is provided, a native button otherwise. Typed as any to
    // forward all props untyped, exactly like the legacy `as` prop did.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const TabButtonComponent: any = to ? LinkAdapter : 'button';
