...
export interface CaptureEnvelope {
  ...
  /** Client-local retry metadata. Senders must not include this on the wire. */
...
export interface CaptureQueueOptions {
  ...
  send(envelope: CaptureEnvelope): Promise<CaptureSendResult>;
  maxAttempts?: number;
  retryBaseMs?: number;
  now?: () => number;
}
...
export interface CaptureQueue {
  enqueue(envelope: CaptureEnvelope): Promise<CaptureQueueEntry>;
