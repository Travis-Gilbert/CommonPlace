...
use commonplace::{
    decide, route, Classification, Commonplace, DeterministicEmbedder, Embedder, InMemoryBlobStore,
    IngestInput, IngestPipeline, Item, ItemKind, NeedsYouReason, OrganizeDecision, OrganizePolicy,
    RoutingRule, SourceRef, TaskFields, COLLECTION_EMBEDDING_PROPERTY, DEFAULT_SOURCE_PRIOR_BOOST,
    ENTITY_LABEL, ITEM_EMBEDDING_PROPERTY, SIMILAR_TO_EDGE,
};
...
fn a3_item_by_source_ref_finds_and_reuses_in_place() {
    ...
    assert!(cp
        .item_by_source_ref("notion", "msg-123")
        .unwrap()
        .is_none());
}
...
fn graph_summary(cp: &Cp) -> GraphSummary {
    ...
    let id_to_title: HashMap<String, String> = items
        .iter()
...
fn intake_inputs() -> Vec<IngestInput> {
    vec![
        ...
        IngestInput::note(
            "Sourdough",
            "Recipe: flour water salt starter, long fermentation.",
        ),
    ]
}
...
fn a4_batch_ingest_graph_is_identical_to_one_at_a_time() {
    ...
    let receipts = pipeline
        .ingest_batch(&mut batched, intake_inputs())
        .unwrap();
...
fn b1_soft_source_prior_breaks_ties_but_content_still_wins() {
    ...
    let rank_of =
        |c: &Classification, id: &str| c.ranked.iter().position(|r| r.collection_id == id).unwrap();
...
fn c1_task_answers_progress_due_and_about_by_graph() {
    ...
    let s1 = cp
        .put_item(Item::task("Find the IBAN", "").with_status("done"))
        .unwrap();
    let s2 = cp
        .put_item(Item::task("Get approval", "").with_status("todo"))
        .unwrap();
    let dep = cp
        .put_item(Item::task("Top up the account", "").with_status("todo"))
        .unwrap();
...
fn c1_task_classifies_into_a_collection_like_a_note() {
    ...
    let receipt = pipeline
        .ingest(
            &mut cp,
            IngestInput::note("Ship the release", "cut the tag and publish notes").as_task(
                TaskFields {
    ...
    assert!(
        !receipt.collection.name.is_empty(),
        "filed into a collection"
    );
