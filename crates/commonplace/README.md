...
Path dep: sibling Theorem checkout's `rustyred-thg-core` until the substrate
crates are published, git-pinned, or adapted behind CommonPlace-owned bridges.

## Build and test

```bash
cargo test --manifest-path crates/commonplace/Cargo.toml
```
...
This crate is CommonPlace-owned product backend code. RustyRed remains the
substrate dependency underneath it.
