...
Standing rules for Claude Code, Codex, and any agent executing work in this repo.
Every handoff assumes these. Read before starting. Each rule maps to a real
...
## Production boundary (SPEC-COMMONPLACE-PRODUCTION-CUTOVER-1.0 GL9)
- The final deliverable of any spec that produces a user-visible surface is its `.commonplace-canonical` manifest row flipped and its production smoke green at a named URL.
- Any spec that supersedes an existing surface carries that surface's deletion as a deliverable with its own acceptance.

## Symphony surface (cutover note)
- No `SPEC-*-SYMPHONY*` file exists in this repository as of 2026-08-03. Until a Symphony surface spec is authored and registered in `.commonplace-canonical`, do not invent a Symphony route, package, or retirement. Record new Symphony work under a named SPEC before code.
...
- Record load-bearing decisions to the harness so they survive across sessions.
...
- Skip the long prose recap of what you did.
...
- Confirm names and contracts against the actual source before writing against them.
