...
| Date | Decision | Why |
|---|---|---|
| 2026-08-15 | Product console alias is `https://apps.theoremweb.com`. Apex `https://theoremweb.com` stays the existing Theseus site. `https://v2.theoremharness.com` remains a live alias. | Travis owns theoremweb.com; the apex already has content, so the CommonPlace console attaches as the `apps` subdomain. |
