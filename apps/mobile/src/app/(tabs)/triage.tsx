/**
 * Mobile Triage: the Needs-you queue and the three life-loop bands. One
 * virtualized column, no counts and no badges;
...
import { drainQueue, getQueueRevision, listPending, subscribeQueue, type CaptureRow } from '@/capture/queue';
...
export default function TriageScreen() {
  ...
  const [searchResults, setSearchResults] = useState<{ query: string; items: ItemGql[] } | null>(null);
  ...
  useEffect(() => {
    ...
    const h = setTimeout(() => {
      searchItems(trimmedSearch, 20)
        .then((hits) => setSearchResults({ query: trimmedSearch, items: hits.map((hit) => hit.item) }))
        .catch(() => setSearchResults({ query: trimmedSearch, items: [] }));
    }, 250);
    return () => clearTimeout(h);
  }, [trimmedSearch]);

  const activeSearchResults = searchResults?.query === trimmedSearch ? searchResults.items : null;

  const sections = (() => {
    if (activeSearchResults) {
      return [{ title: 'Search', data: activeSearchResults.map((item) => ({ type: 'item', item }) as Row) }];
    }
  ...
  return (
    <GestureHandlerRootView style={[styles.root, { backgroundColor: t.c.bg }]}>
      <View style={[styles.topbar, { paddingTop: insets.top + 6 }]}>
        <AppText variant="display1">Triage</AppText>
        <PressableSurface
          onPress={() => router.push('/account')}
          accessibilityLabel="Account"
          style={[styles.avatar, { backgroundColor: t.c.secondary, borderCurve: 'continuous' }]}
