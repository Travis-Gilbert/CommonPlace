// SOURCING: vendored upstream — different-ai/openwork `apps/app/src/app/lib/den.ts`
// @ 2f2dde65796428109a665f3b733843fe3896b933 (MIT). Not a new surface: this file
// arrives with the fork and is edited only to sever Den per
// SPEC-COMMONPLACE-OPENWORK-FORK-1.0 OW1/OW4. No upstream component applies,
// because the change is the removal of a network client, not the addition of one.
import {
...
/**
 * SPEC-COMMONPLACE-OPENWORK-FORK-1.0 OW1 and OW4: Den is severed.
 *
 * Upstream resolved a Den base URL from `VITE_DEN_BASE_URL`, falling back to the
 * hosted control plane at `https://app.openworklabs.com`, and read
 * `VITE_DEN_REQUIRE_SIGNIN` — which `build:web` set to `1` — to force a Den
 * sign-in gate in front of the whole app.
 *
 * Both are now literals rather than env reads, which is load-bearing twice over:
 *   - No default host exists, so no Den request can even be addressed.
 *   - `DEN_ENABLED` is statically `false`, so Vite's dead-code elimination drops
 *     `if (DEN_ENABLED)` branches instead of merely defaulting them off at
 *     runtime. OW4 requires the flags be eliminated from the build.
 *
 * Identity is the console session (OW4). Do not reintroduce an env read here: a
 * cherry-pick that does so silently restores the Den gate and the hosted host.
 */
export const DEN_ENABLED = false as const;
const BUILD_DEN_REQUIRE_SIGNIN = false as const;

export const HOSTED_DEFAULT_DEN_BASE_URL = "";
