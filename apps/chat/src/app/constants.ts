...
export const MCP_QUICK_CONNECT: McpDirectoryInfo[] = [
  ...
  // OW1: two upstream quick-connect entries removed here.
  //
  // "openwork-cloud" was the Den control-plane MCP. Its url getter fell back to
  // https://app.openworklabs.com/api/den/mcp/agent whenever getDenMcpUrl()
  // threw, which — with Den severed — is now always. That fallback was the last
  // live egress path to openworklabs.com in the web bundle.
  //
  // "openwork-ui" spawned `npx -y openwork-ui-mcp`, which would fetch a package
  // from the npm registry at runtime. openwork-ui-mcp is quarried, not adopted
  // (docs/plans/console/openwork-fork/01-VERIFY-FIRST.md §3), so the entry would
  // advertise a surface this fork does not ship.
