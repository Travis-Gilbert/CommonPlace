...
describe("resolveOpencodeModelsUrl", () => {
  ...
  // SPEC-COMMONPLACE-OPENWORK-FORK-1.0 OW1. Upstream defaulted to
  // https://models.openworklabs.com/ here. These three tests are the regression
  // guard: if a cherry-pick reintroduces a third-party default, they fail.
  test("returns no override when nothing is configured", async () => {
    expect(await resolveOpencodeModelsUrl({
      env: {},
  ...
  test("returns no override when the local catalog is unavailable", async () => {
    expect(await resolveOpencodeModelsUrl({
      env: { OPENWORK_DEV_MODE: "1" },
      fetchModels: async () => new Response(null, { status: 503 }),
    })).toBe("");
  ...
  test("never resolves to a third-party host without explicit opt-in", async () => {
    for (const env of [{}, { OPENWORK_DEV_MODE: "1" }, { OPENWORK_DEV_MODE: "0" }]) {
      const resolved = await resolveOpencodeModelsUrl({
        env,
        fetchModels: async () => new Response(null, { status: 503 }),
      });
      expect(resolved).not.toContain("openworklabs.com");
    }
  });
});
