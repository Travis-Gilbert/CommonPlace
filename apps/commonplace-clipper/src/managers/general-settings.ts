...
import { SaveBehavior, Settings, Template } from '../types/types';
...
const STORE_URLS = {
	chrome: 'https://v2.theoremharness.com',
	firefox: 'https://v2.theoremharness.com',
	safari: 'https://v2.theoremharness.com',
	edge: 'https://v2.theoremharness.com'
};
...
function initializeCommonplaceSettings(): void {
	const endpointInput = document.getElementById('commonplace-endpoint-url') as HTMLInputElement;
	const tokenInput = document.getElementById('commonplace-api-token') as HTMLInputElement;

	if (endpointInput) {
		endpointInput.value = generalSettings.commonplaceEndpointUrl;
		endpointInput.addEventListener('change', () => {
			saveSettings({ commonplaceEndpointUrl: endpointInput.value.trim() });
		});
	}

	if (tokenInput) {
		tokenInput.value = generalSettings.commonplaceApiToken;
		tokenInput.addEventListener('change', () => {
			saveSettings({ commonplaceApiToken: tokenInput.value.trim() });
		});
	}
}
