...
export async function exportHighlights(): Promise<void> {
	try {
		...
		const fileName = `commonplace-web-clipper-highlights-${timestamp}.json`;
			...
			if (navigator.share) {
				try {
					await navigator.share({
						...
						title: 'Exported CommonPlace Web Clipper Highlights',
						text: 'Here are your exported highlights from CommonPlace Web Clipper.'
					});
				} catch (error) {
