...
import {
	COMMONPLACE_CAPTURE_ALARM,
	drainCommonplaceCaptures,
	enqueueCommonplaceCapture,
	type CaptureEnvelope,
} from './utils/commonplace-capture';
...
browser.runtime.onMessage.addListener((request: unknown) => {
	if (typeof request !== 'object' || request === null) return;
	const captureRequest = request as {
		action?: string;
		envelope?: CaptureEnvelope;
	};
	if (captureRequest.action !== 'enqueueCommonplaceCapture') return;
	if (!captureRequest.envelope?.client_id) {
		return Promise.resolve({
			ok: false,
			error: 'Capture envelope client_id is required',
		});
	}
	return enqueueCommonplaceCapture(captureRequest.envelope)
		.catch((error) => ({
			ok: false,
...
function drainCaptureQueueFromBackground(): void {
	void drainCommonplaceCaptures().catch((error) => {
		console.warn(
			'[CommonPlace Clipper] Capture queue drain deferred:',
			error instanceof Error ? error.message : String(error),
		);
	});
}

function scheduleCaptureDrain(): void {
	if (!browser.alarms) return;
	void browser.alarms.create(COMMONPLACE_CAPTURE_ALARM, {
		periodInMinutes: 1,
	});
}
...
browser.runtime.onInstalled.addListener(() => {
	...
	drainCaptureQueueFromBackground();
});
...
if (browser.alarms) {
	browser.alarms.onAlarm.addListener((alarm) => {
		if (alarm.name === COMMONPLACE_CAPTURE_ALARM) {
			drainCaptureQueueFromBackground();
		}
	});
}
