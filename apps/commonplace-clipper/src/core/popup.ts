...
import { generateFrontmatter, saveToCommonplace, saveToObsidian } from '../utils/obsidian-note-creator';
			...
			button.addEventListener('click', async (e) => {
				...
				Promise.all([
					...
					if (navigator.share && navigator.canShare) {
						...
						const shareData = {
							files: [file],
							text: 'Shared from CommonPlace Web Clipper'
						};
...
function determineMainAction() {
	...
	switch (loadedSettings.saveBehavior) {
		case 'saveToCommonplace':
			mainButton.textContent = getMessage('saveToCommonplace');
			mainButton.onclick = () => handleClipCommonplace(false);
			addSecondaryAction(secondaryActions, 'addToObsidian', () => handleClipObsidian());
			addSecondaryAction(secondaryActions, 'saveToBoth', () => handleClipCommonplace(true));
			...
			break;
		case 'saveToBoth':
			mainButton.textContent = getMessage('saveToBoth');
			mainButton.onclick = () => handleClipCommonplace(true);
			addSecondaryAction(secondaryActions, 'saveToCommonplace', () => handleClipCommonplace(false));
			addSecondaryAction(secondaryActions, 'addToObsidian', () => handleClipObsidian());
			addSecondaryAction(secondaryActions, 'copyToClipboard', copyContent);
			addSecondaryAction(secondaryActions, 'saveFile', handleSaveToDownloads);
			break;
...
async function handleClipCommonplace(includeObsidian: boolean): Promise<void> {
	...
	if (!loadedSettings.commonplaceEndpointUrl) {
		showError('commonplaceEndpointMissing');
		return;
	}
	...
	try {
		...
		const noteName = isDailyNote ? tabTitle : noteNameField?.value || tabTitle;
		const path = isDailyNote ? '' : pathField?.value || '';

		await saveToCommonplace(
			fileContent,
			noteName,
			properties,
			tabInfo.url,
		...
		if (includeObsidian) {
			await saveToObsidian(fileContent, isDailyNote ? '' : noteName, path, selectedVault, currentTemplate.behavior);
		...
		console.error('Error in handleClipCommonplace:', error);
		showError('failedToSaveFile');
		throw error;
	}
}
...
function getActionIcon(actionType: string): string {
	switch (actionType) {
		...
		case 'saveToCommonplace': return 'book-open';
		case 'saveToBoth': return 'copy-plus';
		default: return 'plus';
	}
}
