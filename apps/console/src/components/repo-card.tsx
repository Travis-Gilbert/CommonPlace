...
/** The muted description slot (a repo card's description line). */
export function RepoCardDescription({ children }: { readonly children: ReactNode }) {
  return <p className="text-sm text-ij-ink-info">{children}</p>;
}
...
export function RepoCard({
  ...
  return (
    <div
      data-node={dataNode}
      className="flex flex-col gap-3 rounded-ij-arc border border-ij-seam-raised bg-ij-editor p-4 hover:border-ij-control-border"
    >
