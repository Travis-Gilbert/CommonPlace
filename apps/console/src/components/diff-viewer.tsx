...
function lineColor(type: DiffLine["type"], element: "bg" | "text" | "num") {
  if (type === "added") {
    return element === "bg"
      ? "bg-ij-ok/10 dark:bg-ij-ok/10"
      : element === "num"
        ? "text-ij-ok/70 dark:text-ij-ok/50"
        : "text-ij-ok dark:text-ij-ok"
  }
  if (type === "removed") {
    return element === "bg"
      ? "bg-ij-error/10 dark:bg-ij-error/10"
      : element === "num"
        ? "text-ij-error/70 dark:text-ij-error/50"
        : "text-ij-error dark:text-ij-error"
  }
  return element === "num"
        ...
        <div className="flex items-center justify-between gap-3 border-b border-border/60 bg-muted/40 px-4 py-2.5">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 text-xs">
              {stats.added > 0 && (
                <span className="text-ij-ok dark:text-ij-ok">
                  +{stats.added}
                </span>
              )}
              {stats.removed > 0 && (
                <span className="text-ij-error dark:text-ij-error">
                  -{stats.removed}
                </span>
              )}
            </div>
