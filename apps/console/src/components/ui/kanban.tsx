// SOURCING: keenthemes/reui kanban primitive
// (registry-reui/bases/base/reui/kanban.tsx), structure and sortable behavior
// ported verbatim onto @base-ui/react (the same primitives layer the
// twenty-ui fork uses) plus @dnd-kit/core + @dnd-kit/sortable; paint resolves
// to the register bridge. Registry-install file, so metric arbitrary values
// are legal here; colors stay register-resolved.
'use client';
 ...
 * snapshot (`false`) while rendering on the server and while hydrating, then
 * the client snapshot (`true`) once mounted. All three functions are
 * module-scoped so their identities stay stable; an inline `getSnapshot` is
 * the classic cause of an infinite re-subscribe loop.
 */
...
function KanbanColumn({
  ...
  // Hooks must run unconditionally; the derived value below is used only in
  // the non-overlay branch.
