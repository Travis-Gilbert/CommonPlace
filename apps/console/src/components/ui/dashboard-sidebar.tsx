...
/* Widened from LucideIcon so the product nav can carry Noun Project marks
   (Icons ledger row) while the vendor's own demo tree keeps its lucide
   glyphs. Both render as <item.icon className strokeWidth />, so the
   published render site is untouched. */
  ...
  return (
    <div className="relative pointer-events-auto">
      ...
      >
        ...
        <IconChevronDown className="w-4 h-4 text-muted-foreground/50 group-hover:text-foreground/70 transition-colors shrink-0" />
      </div>
...
/* Icons ledger row: the product nav paints Noun Project marks from the one
   icon file. The lucide glyphs below this line belong to the vendor's own
   demo tree, which no product surface renders. Both satisfy NavIcon, so the
   published render site is untouched. */
const PLACE_ICONS: Record<string, NavIcon> = {
  chat: IconChat,
  survey: IconCards,
  index: IconIndex,
...
function buildPlaceNavGroup(): NavGroupData {
  return {
    ...
    items: PLACE_ENTRIES.map((place) => ({
      id: place.path,
      title: place.label,
      icon: PLACE_ICONS[place.kind] ?? IconRail,
    })),
  };
}
    ...
    <div className="flex flex-col w-full">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          {hasChildren && (
            <IconChevronDown
              className={`w-3.5 h-3.5 text-muted-foreground/50 transition-transform duration-200 ${isOpen ? 'rotate-0' : '-rotate-90'}`}
            />
          )}
        </div>
      </div>
        ...
        <div className="flex-1 bg-black/[0.02] dark:bg-white/[0.02] flex flex-col min-w-0 transition-all duration-300">
           ...
           <div className="h-14 border-b border-border/50 flex items-center px-4 justify-between bg-card shrink-0">
             <div className="flex items-center gap-3">
               ...
               >
                 {isOpen ? <IconHide className="w-[18px] h-[18px]" /> : <IconRail className="w-[18px] h-[18px]" />}
               </button>
          ...
          <div className="absolute inset-0 z-50 flex items-start justify-center pt-[15vh] bg-background/40 backdrop-blur-sm px-4">
            ...
            <div className="relative w-full max-w-xl bg-card border border-border/50 rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              <div className="flex items-center px-4 border-b border-border/50">
                <IconSearch className="w-[18px] h-[18px] text-muted-foreground/70 mr-3 shrink-0" />
                <input 
                  autoFocus
              ...
              <div className="p-2 py-8 flex flex-col items-center justify-center">
                 <IconCommand className="w-6 h-6 text-muted-foreground/30 mb-2" />
