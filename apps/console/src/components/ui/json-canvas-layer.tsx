...
export function JsonCanvasLayer({
  ...
  useEffect(() => {
    ...
    /* readyCanvas reaches the objects API, which can be down or unreachable.
       A bare `void boot()` turned that into an unhandled rejection that took
       the dev overlay with it, so the whole inspector rail read as broken when
       only its canvas was. Degrade to an empty canvas and say why. */
    void boot().catch((error: unknown) => {
      if (cancelled) return;
      applyLocal(EMPTY_CANVAS);
  ...
  return (
    ...
    >
      ...
      {/* This layer is full-bleed under the rail's nav, so an overlay pinned to
          top-3 lands on the workspace switcher and one pinned to bottom-3 lands
          on the Settings row, with translucent fills letting both strings read
          through each other. Clear the two chrome rows and paint opaque.
          ponytail: the offsets are grid steps chosen to clear today's header
          and footer; if either row grows, hand the real heights down from
          DashboardSidebar instead of widening these. */}
      {document.nodes.length === 0 ? (
        <p
          className="pointer-events-none absolute inset-x-3 bottom-14 z-20 rounded-ij-arc bg-ij-raised px-2 py-1.5 text-ij-island-meta text-ij-ink-info"
      ...
      {persistError ? (
        <p
          className="pointer-events-none absolute inset-x-3 top-16 z-20 rounded-ij-arc border border-ij-seam bg-ij-raised px-2 py-1.5 text-ij-island-meta text-ij-error"
