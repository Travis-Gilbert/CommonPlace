...
function TokenSpan({
  ...
  const fallbackMap: Record<string, string> = {
    key: "text-ij-graph dark:text-ij-graph",
    string: "text-ij-ok dark:text-ij-ok",
    number: "text-ij-running dark:text-ij-running",
    boolean: "text-ij-warn dark:text-ij-warn",
...
function HighlightMatch({
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark className="rounded-sm bg-ij-warn/60 px-0.5 text-inherit dark:bg-ij-warn/30">
...
function JsonNode({
  ...
  const rowClass = cn(
    ...
    !theme && nodeMatches && "bg-ij-warn/40 dark:bg-ij-warn/20"
  )
  ...
  if (!isExpandable) {
    return (
      ...
      >
        ...
        >
          {pathCopied ? (
            <CopyPlus className="size-3 text-ij-ok" />
          ) : (
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-border/40 px-3 py-2 sm:px-4">
          ...
          <div className="flex items-center gap-0.5">
            ...
            >
              {copiedAll ? (
                <Check className="size-3.5 text-ij-ok" />
              ) : (
