...
export type LevelColors = {
  /** CSS class for the level label text (e.g. "text-ij-error dark:text-ij-error"). */
  text: string
  /** CSS class for the colored dot (e.g. "bg-ij-error"). */
  dot: string
  /** CSS class for the filter badge when active (e.g. "bg-ij-error/15 text-ij-error"). */
  badge: string
}
...
const DEFAULT_LEVEL_COLORS: Record<LogLevel, LevelColors> = {
  error: {
    text: "text-ij-error dark:text-ij-error",
    dot: "bg-ij-error",
    badge: "bg-ij-error/15 text-ij-error dark:text-ij-error",
  },
  warn: {
    text: "text-ij-warn dark:text-ij-warn",
    dot: "bg-ij-warn",
    badge: "bg-ij-warn/15 text-ij-warn dark:text-ij-warn",
  },
  info: {
    text: "text-ij-running dark:text-ij-running",
    dot: "bg-ij-running",
    badge: "bg-ij-running/15 text-ij-running dark:text-ij-running",
  },
  debug: {
    text: "text-ij-graph dark:text-ij-graph",
    dot: "bg-ij-graph",
    badge: "bg-ij-graph/15 text-ij-graph dark:text-ij-graph",
  },
  verbose: {
    text: "text-ij-ink-info dark:text-ij-ink-info",
    dot: "bg-ij-ink-disabled dark:bg-ij-ink-disabled",
...
function highlightSearch(text: string, query: string): React.ReactNode {
  ...
  return parts.map((part, i) =>
    part.toLowerCase() === query.toLowerCase() ? (
      <mark key={i} className="rounded-sm bg-ij-warn/40 px-0.5 text-inherit dark:bg-ij-warn/30">
        {part}
      </mark>
    ) : (
      part
      ...
      <div className="flex items-center gap-2 border-b border-border/40 bg-muted/30 px-3 py-2">
        ...
        <div className="flex items-center gap-0.5">
          ...
          <ToolbarButton onClick={handleCopyAll} label={copied ? "Copied" : "Copy all logs"}>
            {copied ? <Check className="size-3.5 text-ij-ok" /> : <Copy className="size-3.5" />}
          </ToolbarButton>
