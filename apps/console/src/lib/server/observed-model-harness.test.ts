...
describe('Console Models consumer GraphQL transport', () => {
  ...
  it('ignores an agent endpoint and uses the authenticated local stand-in outside production', async () => {
    ...
    expect(result.tenant).toBe('Travis-Gilbert');
    expect(result.declared.scope.tenant).toBe('Travis-Gilbert');
    expect(result.declared.objectTypes.map((type) => type.key)).toContain('customer');
    expect(fetchMock).not.toHaveBeenCalled();
    expect(resolveHarnessPrincipalMock).toHaveBeenCalledTimes(1);
  });

  it('fails closed when the consumer endpoint is unset in production', async () => {
    vi.stubEnv('NODE_ENV', 'production');

    const result = await readObservedModels('topic-production');
  ...
  it('refuses the local stand-in when principal resolution fails', async () => {
    resolveHarnessPrincipalMock.mockResolvedValue({
      ok: false,
      response: Response.json({ error: 'unauthenticated' }, { status: 401 }),
    });

    const result = await readObservedModels('topic-anonymous');
  ...
  it('rejects a successful GraphQL envelope missing the requested fields', async () => {
    vi.stubEnv('CONSOLE_DATA_API_URL', 'https://data.example/graphql');
    fetchMock.mockResolvedValue(graphqlResponse({ unrelated: true }));
    ...
    expect(result).toEqual(expect.objectContaining({
      ok: false,
      status: 502,
      error: 'observed_model_graphql_invalid_response:observedModel',
