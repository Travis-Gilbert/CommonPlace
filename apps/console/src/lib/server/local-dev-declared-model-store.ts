...
const DEFAULT_LOCAL_TENANT = 'local-dev';
...
function scopeFor(topicId: string, tenant: string): ScopeRef {
  return { kind: 'topic', topicId, tenant };
}
...
function starterDeclared(topicId: string, tenant: string): DeclaredModel {
...
function ensureBucket(topicId: string, tenant: string): TopicBucket {
  const bucketKey = `${tenant}\u0000${topicId}`;
  const existing = buckets.get(bucketKey);
  if (existing) return existing;
  const declared = starterDeclared(topicId, tenant);
  const bucket: TopicBucket = {
    declared,
    observed: emptyObservedModel(scopeFor(topicId, tenant)),
    versionSeq: 1,
  };
...
export function readLocalDevModels(
  topicId: string,
  tenant = DEFAULT_LOCAL_TENANT,
): {
...
export function unpinLocalDevDeclared(
  ...
  return { tenant, declared: bucket.declared };
}
...
export function emptyLocalDevDeclared(
  topicId: string,
  tenant = DEFAULT_LOCAL_TENANT,
): DeclaredModel {
  return emptyDeclaredModel(scopeFor(topicId, tenant));
}
