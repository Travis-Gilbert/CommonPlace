...
| Graph node and sentence card building blocks | jalco-ui `@jalco/commit-graph` and `@jalco/repo-card` (ui.justinlevine.me), **adopted** through the shadcn registry declared in `components.json`, MIT (jal-co/ui `LICENSE`, blob SHA `160c1593d90acd73205bd976ef11a66c1986dfa0`) | the commit row every graph node renders (`CommitRow`), the rail cubic every edge is drawn with (`railPath`), the popover detail, and the card grammar the whole card altitude is built from (`repoCardVariants` and its slots). Entered by `shadcn add @jalco/commit-graph @jalco/repo-card`, then retokened in place: upstream's eight-color rail cycle becomes the speaker registers, every shadcn token becomes an `--ij-*`/`--cp-*` token, and the deviations (no GitHub glyphs per decision 12, no GitHub fetch, `div` root, no shadows) are named in each file's header. Reproduction is no longer the mode; the files ARE upstream's. |
| Class merging for adopted components | `clsx` plus `tailwind-merge`, through shadcn's canonical `cn` in `src/lib/utils.ts` | conditional class expressions and the later-wins conflict resolution a CVA component needs so a caller's `className` actually overrides a variant |
| Component variants | `class-variance-authority` | the variant matrices adopted registry components declare (`repoCardVariants`) |
| Popover primitive for adopted components | `radix-ui` (the unified package the jalco registry item declares) | the commit-detail popover shipped inside `@jalco/commit-graph` |
