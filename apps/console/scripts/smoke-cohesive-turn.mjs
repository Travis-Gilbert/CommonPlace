...
function stateFromTransportEvent(data) {
  if (!data || typeof data !== 'object' || data.type !== 'update-state') return null;
  const operation = Array.isArray(data.operations)
    ? data.operations.find((candidate) => (
        candidate?.type === 'set' &&
        Array.isArray(candidate.path) &&
        candidate.path.length === 0
      ))
    : null;
  return operation?.value && typeof operation.value === 'object' ? operation.value : null;
}

async function runTransportTurn({ baseUrl, cookie, prompt }) {
  const started = performance.now();
  const response = await fetch(new URL('/api/chat/transport', baseUrl), {
    method: 'POST',
    headers: {
      Cookie: cookie,
      Accept: 'text/event-stream',
  ...
  if (!response.ok) {
    const body = (await response.text()).slice(0, MAX_ERROR_BODY);
    throw new Error(`assistant_transport returned HTTP ${response.status}: ${body}`);
  }
  const turnId = response.headers.get('x-commonplace-turn-id');
  if (!turnId) throw new Error('assistant_transport omitted x-commonplace-turn-id.');
  if (response.headers.get('x-commonplace-turn-mode') !== 'cohesive') {
    throw new Error('assistant_transport did not reach the cohesive path.');
  }
  if (!response.body) throw new Error('assistant_transport returned no event stream.');
  ...
  let finalState = null;
  let firstSubstantiveTokenMs = null;
  const decoder = new TextDecoder();
  const reader = response.body.getReader();
  for (;;) {
    const { done, value } = await reader.read();
    ...
    for (;;) {
      ...
      const event = parseEvent(buffer.slice(0, boundary));
      buffer = buffer.slice(boundary + 2);
      const state = stateFromTransportEvent(event?.data);
      if (!state) continue;
      finalState = state;
      sawRunning ||= state.turnStatus === 'running';
      sawRunningActivity ||= state.activityStatus === 'running';
      const assistant = Array.isArray(state.messages)
        ? [...state.messages].reverse().find((message) => message?.role === 'assistant')
        : null;
      if (typeof assistant?.acknowledgement === 'string') {
        acknowledgement ??= assistant.acknowledgement;
      }
      if (typeof assistant?.text === 'string' && assistant.text.trim()) {
        firstSubstantiveTokenMs ??= Math.round((performance.now() - started) * 100) / 100;
      }
    }
    if (done) break;
  }

  if (!sawRunning) throw new Error('assistant_transport emitted no running turn state.');
  if (!sawRunningActivity) throw new Error('assistant_transport emitted no real running activity.');
  if (!acknowledgementIsSpecific(acknowledgement)) {
    throw new Error('assistant_transport published a generic or overlong acknowledgement.');
  }
  if (!finalState || (finalState.turnStatus !== 'complete' && finalState.turnStatus !== 'refused')) {
    throw new Error(`assistant_transport ended in ${finalState?.turnStatus ?? 'no state'}.`);
  }
  const answer = Array.isArray(finalState.messages)
    ? [...finalState.messages].reverse().find((message) => message?.role === 'assistant')?.text
    : null;
  if (typeof answer !== 'string' || !answer.trim()) {
    throw new Error('assistant_transport emitted no substantive composed response.');
  }

  return {
    turn_id: turnId,
    acknowledgement_published: typeof acknowledgement === 'string',
    response_sha256: createHash('sha256').update(answer).digest('hex'),
    first_substantive_token_ms: firstSubstantiveTokenMs,
    completion_ms: Math.round((performance.now() - started) * 100) / 100,
  };
}
...
async function main() {
  ...
  const assistantTransportReceipt = await runTransportTurn({
    baseUrl,
    cookie,
    prompt: 'Explain this connected workspace in one concise sentence.',
  });
