...
> Placement translation note, 2026-07-26: `apps/console` is now the canonical
> product host. Read every `apps/web` path in this retained spec as its
> `apps/console` counterpart. Canvas node paint lands in
> `src/views/canvas/CanvasCardNode.tsx`; register primitives and tokens land
> under `src/styles/`. The body below is preserved as written.
