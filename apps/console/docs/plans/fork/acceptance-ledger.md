...
| FK11 Retirement | accepted | CN1 records a verdict for every Console view. The harvested search stack and shadcn primitives pass their gates. `apps/web` is deleted, the root deployment builds Console, all 62 remaining view files are reachable, old seeded view trees are removed, and `/v/chat` returns an HTTP 404. The full browser run passes 82 with 4 scoped-Chat cases and 1 live-service case intentionally skipped. | The production redirect and authenticated deployed one-shell flow still need operational proof, but no dead source routing path remains. |
...
| Harvested console is equal-or-better | accepted | Full Console gates, production build, and 82 passing browser tests establish the local replacement. |
...
- Playwright: 82 passed, 4 scoped-Chat cases skipped, and 1 live-service integration case skipped.
