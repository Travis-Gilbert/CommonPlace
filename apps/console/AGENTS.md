...
| Sortable kanban board | keenthemes/reui kanban primitive (`registry-reui/bases/base/reui/kanban.tsx`) on `@base-ui/react` (the same primitives layer the twenty-ui fork uses) plus `@dnd-kit/core` + `@dnd-kit/sortable` | `ui/kanban.tsx`: card sort within and across columns, keyboard sensor, overlay, commit metadata, useRender/mergeProps polymorphism; register-skinned |
| Kanban board 5 block | keenthemes/reui `c-kanban-5.tsx` (kanban-board-5), structure, behavior, and fixture data | `ui/kanban-board-5.tsx`: status-dot columns with `twenty-ui/data-display` Pill counts, raised cards with `twenty-ui/feedback` ProgressBar (`--t-*` themed, fill on `--ij-progress`), Tabler votes via `twenty-ui/icon`; the product `views/blocks/KanbanBlock` adapts the same anatomy to nested view-instances and keeps typed containment (AMENDMENT-02 A2-1/A2-2) |
| App shell 8 block | keenthemes/reui app-shell-8 (live preview DOM; the app-shell blocks are licensed and absent from the public repo), structure, behavior, and fixture data | `ui/app-shell-8.tsx`: two-level app shell (icon rail, per-section secondary nav, env-switcher breadcrumb, command search, create menu, notifications sheet, storage usage bar, mobile nav sheet) on the local radix/`ui/*` primitives plus `twenty-ui/data-display` Avatar and Pill counts and `twenty-ui/feedback` ProgressBar on `--ij-warn`, Tabler glyphs via `twenty-ui/icon` (Package, Shopping cart, Truck, Arrows left right, Chevrons up down map to fork near-equivalents, listed in the file header); register-skinned, not yet mounted to a surface |
