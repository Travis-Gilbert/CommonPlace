/**
 * Five-state view discipline (SPEC-UX-PHYSICS D4). Every read-bearing surface
 ...
 * while the rest streams. The wait-tier ladder (commonplace-wait-tier) refines
 * the `loading` branch; it does not replace this union.
 *
 * Mobile mirrors this module semantically in apps/mobile (same five statuses).
 */
 ...
 * is the seam that lets local-first reads (SPEC-UX-PHYSICS D2) render `partial`
