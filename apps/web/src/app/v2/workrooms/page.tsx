...
/* Workroom Control Center (/v2, porcelain).
 *
 * The operational face of Workrooms, Approvals, and Receipts, re-seated on the
 * porcelain shell. Logic and contract are unchanged from the legacy surface:
 * one GET to /api/theorem/control-center yields the whole state; typed POST
 * actions return durable receipts; every card degrades honestly via source.mode
 * until Codex swaps fixtures for live Theorem substrate.
 *
 * Parity: PT-003 Workrooms, PT-004 Receipts, PT-005 Approvals, PT-006 Routes,
 * PT-007 Setup, PT-008 Commands, PT-009 Memory, PT-010 Tools, PT-011 Skill
 * candidates, PT-012 Reconstruction, PT-013 Eval. */
...
import styles from './workrooms.module.css';
...
type Tone = 'ok' | 'attention' | undefined;

/* porcelain is restrained: green (ready), rubric-red (attention/danger), ink. */
function statusTone(status: ControlCenterStatus | string): Tone {
  switch (status) {
    case 'ready':
    case 'passed':
    case 'approved':
...
function riskTone(risk: RiskClass): Tone {
  return risk === 'destructive' ? 'attention' : undefined;
}
function tierTone(tier: string): Tone {
  return tier === 'trusted_local' ? 'ok' : tier === 'denied' ? 'attention' : undefined;
}
  ...
  return (
    <>
      ...
      <div className={styles.body}>
        ...
        {error && <div className={styles.errorCard}>Control center unavailable: {error.message}</div>}
        {loading && !data && <div className={styles.loading}>Loading control center...</div>}
        {data && (
          <>
            {section === 'workrooms' && <Workrooms data={data} busy={busy} run={runAction} />}
            {section === 'approvals' && <Approvals items={data.approvals} busy={busy} run={runAction} />}
            {section === 'receipts' && <Receipts items={data.receipts} />}
            {section === 'routes' && <Routes items={data.routes} busy={busy} run={runAction} />}
            {section === 'memory' && <Memory memory={data.memory} busy={busy} run={runAction} />}
            {section === 'tools' && (
              <Tools
                tools={data.tools}
            ...
            {section === 'setup' && <Setup items={data.setup} busy={busy} run={runAction} />}
...
function Workrooms({ data, busy, run }: { data: { workrooms: WorkroomSummary[] }; busy: string | null; run: Runner }) {
            ...
            return (
              <article key={w.id} className={styles.card}>
                ...
                <div className={styles.cardMeta}>
                  ...
                  <span className={`${styles.mono} ${styles.muted}`}>{w.artifactCount} artifacts</span>
                  {w.eval && <Pill tone={statusTone(w.eval.status)}>eval: {w.eval.status}</Pill>}
                  {w.fitness != null && <span className={`${styles.mono} ${styles.muted}`}>fitness {w.fitness}</span>}
                ...
                <div className={styles.actions}>
                  ...
                  <button
                    ...
                    onClick={() => run({ action: 'eval_trace_export', workroomId: w.id }, `eval:${w.id}`)}
                  >
                    export trace
                  </button>
                </div>
...
function Approvals({ items, busy, run }: { items: ApprovalCard[]; busy: string | null; run: Runner }) {
      ...
      {items.length === 0 ? (
        ...
        <div className={`${styles.grid} ${styles.gridCols}`}>
          {[...pending, ...history].map((a) => {
            ...
            return (
              <article key={a.id} className={`${styles.card} ${isPending ? styles.cardPending : ''}`}>
                <div className={styles.cardHead}>
                  <span className={styles.cardTitle}>{a.title}</span>
                  <Pill tone={statusTone(a.status)}>{a.status}</Pill>
                </div>
                <div className={styles.cardSummary}>{a.summary}</div>
                {a.diffSummary && <div className={`${styles.mono} ${styles.muted}`} style={{ marginTop: 6 }}>{a.diffSummary}</div>}
                {a.commandSummary && <div className={`${styles.mono} ${styles.muted}`} style={{ marginTop: 6 }}>$ {a.commandSummary}</div>}
                <div className={styles.cardMeta}>
                  ...
                  <div className={styles.actions}>
                    {a.actions.includes('approve_once') && (
                      <button
                        ...
                        onClick={() => run({ action: 'approval_decision', approvalId: a.id, decision: 'approve_once' }, `ap:${a.id}`)}
                      >
                        approve once
                      </button>
                    )}
                    {a.actions.includes('remember') && (
                      <button
                        ...
                        onClick={() =>
                          run(
                            { action: 'approval_decision', approvalId: a.id, decision: 'remember', rememberScope: `${a.toolName}:${a.target}` },
                            `ap:${a.id}`,
                          )
                        }
                      >
                    ...
                    {a.actions.includes('deny') && (
                      <button
                        ...
                        disabled={busy === `ap:${a.id}`}
                        onClick={() => run({ action: 'approval_decision', approvalId: a.id, decision: 'deny' }, `ap:${a.id}`)}
                      >
                        deny
                      </button>
                    )}
              ...
              {r.inspect && (
                <div className={styles.kv}>
                  ...
                  <span className={styles.kvVal}>
                    {r.inspect.href ? <a href={r.inspect.href} className={styles.mono}>{r.inspect.label}</a> : r.inspect.label}
                    {r.inspect.bodyPreview && (
                      <div className={`${styles.mono} ${styles.dim}`} style={{ marginTop: 4 }}>{r.inspect.bodyPreview}</div>
                    )}
                  </span>
                </div>
              )}
              {r.changed.length > 0 && (
                <div className={styles.chips}>{r.changed.map((c) => <span key={c} className={styles.chip}>{c}</span>)}</div>
              )}
              {r.recovery.length > 0 && (
                <div className={styles.chips}>{r.recovery.map((c) => <span key={c} className={styles.chip}>↺ {c}</span>)}</div>
              )}
              {r.provenance.length > 0 && (
                <div className={`${styles.mono} ${styles.dim}`} style={{ marginTop: 8 }}>{r.provenance.join(' · ')}</div>
              )}
            </article>
          ))}
        </div>
...
function Routes({ items, busy, run }: { items: RouteCard[]; busy: string | null; run: Runner }) {
  return (
    <section>
      <SectionHead
        ...
        <div className={`${styles.grid} ${styles.gridCols}`}>
          {items.map((r) => (
            <article key={r.id} className={styles.card}>
              ...
              <div className={styles.kv}>
                <span className={styles.kvKey}>provider</span><span className={styles.kvVal}>{r.provider}</span>
                <span className={styles.kvKey}>model</span><span className={`${styles.kvVal} ${styles.mono}`}>{r.model}</span>
                <span className={styles.kvKey}>runtime</span><span className={styles.kvVal}>{r.runtime}</span>
                <span className={styles.kvKey}>channel</span><span className={styles.kvVal}>{r.channel}</span>
                <span className={styles.kvKey}>protocol</span><span className={styles.kvVal}>{r.protocol} · {r.endpointKind}</span>
                <span className={styles.kvKey}>auth</span><span className={styles.kvVal}>{r.authState}</span>
                <span className={styles.kvKey}>context</span>
                <span className={styles.kvVal}>{r.contextWindow != null ? `${r.contextWindow.toLocaleString()} tok` : 'unknown'}</span>
              ...
              <div className={styles.actions}>
                <SourceBadge source={r.source} />
                <button className={styles.btn} disabled={busy === `rt:${r.id}`} onClick={() => run({ action: 'route_check', routeId: r.id }, `rt:${r.id}`)}>
                  check route
                </button>
              </div>
            </article>
...
function Memory({ memory, busy, run }: { memory: { contextBundles: ContextBundle[]; blocks: MemoryBlock[] }; busy: string | null; run: Runner }) {
  return (
    <section>
      <SectionHead
                ...
                <article key={b.id} className={styles.card}>
                  ...
                  {b.admittedSources.map((s) => (
                    <div key={s.id} className={styles.kv}>
                      ...
                      <span className={styles.kvVal}>
                        {s.reason}
                        <div className={`${styles.mono} ${styles.dim}`} style={{ marginTop: 2 }}>{s.provenance.join(' · ')}</div>
                      </span>
                    </div>
                  ))}
                </article>
            ...
            <div className={styles.grid}>
              {memory.blocks.map((m) => (
                <article key={m.id} className={styles.card}>
                  ...
                  <div className={`${styles.mono} ${styles.dim}`} style={{ marginTop: 6 }}>{m.provenance.join(' · ')}</div>
                  <div className={styles.actions}>
                    {m.actions.map((act) => (
                      <button
                        key={act}
                        ...
                        onClick={() => run({ action: 'memory_action', blockId: m.id, memoryAction: act }, `mem:${m.id}`)}
                      >
                        {act}
                      </button>
                    ))}
          ...
          <div className={`${styles.grid} ${styles.gridCols}`}>
            {tools.map((t) => (
              <article key={t.id} className={styles.card}>
                ...
                <div className={styles.cardMeta}>
                  <Pill>{t.kind}</Pill>
                  {t.oauthRequired && <Pill tone="attention">oauth</Pill>}
                  <SourceBadge source={t.source} />
                </div>
                <div className={styles.cardSummary}>{t.permissionSummary}</div>
                {t.envHints.length > 0 && (
                  <div className={styles.chips}>{t.envHints.map((e) => <span key={e} className={styles.chip}>{e}</span>)}</div>
                )}
                <div className={styles.actions}>
                  <button className={styles.btn} disabled={busy === `tool:${t.id}`} onClick={() => run({ action: 'tool_health_check', toolId: t.id }, `tool:${t.id}`)}>
                    health check
                  </button>
                </div>
              </article>
            ...
            <div className={styles.grid}>
              {candidates.map((c) => (
                <article key={c.id} className={styles.card}>
                  ...
                  <div className={styles.cardMeta}>
                    <Pill>{c.gate}</Pill>
                    {c.fitness != null && <span className={`${styles.mono} ${styles.muted}`}>fitness {c.fitness}</span>}
                  </div>
                  <div className={styles.actions}>
                    <button className={styles.btn} disabled={busy === `sk:${c.id}`} onClick={() => run({ action: 'skill_candidate_action', candidateId: c.id, decision: 'shadow_eval' }, `sk:${c.id}`)}>
                      shadow eval
                    </button>
                    <button className={styles.btnPrimary} disabled={busy === `sk:${c.id}`} onClick={() => run({ action: 'skill_candidate_action', candidateId: c.id, decision: 'approve' }, `sk:${c.id}`)}>
                      approve
                    </button>
                    <button className={styles.btnDanger} disabled={busy === `sk:${c.id}`} onClick={() => run({ action: 'skill_candidate_action', candidateId: c.id, decision: 'reject' }, `sk:${c.id}`)}>
                      reject
                    </button>
                  </div>
                </article>
            ...
            <div className={styles.grid}>
              {packets.map((p) => (
                <article key={p.id} className={styles.card}>
                  ...
                  {p.evidence.length > 0 && (
                    <div className={styles.chips}>{p.evidence.map((e) => <span key={e} className={styles.chip}>{e}</span>)}</div>
                  )}
                </article>
              ))}
            </div>
...
function Setup({ items, busy, run }: { items: SetupCheck[]; busy: string | null; run: Runner }) {
  return (
    <section>
      <SectionHead
        title="Setup & Doctor"
      ...
      />
      <div style={{ marginBottom: 'calc(var(--u) * 2)' }}>
        <button className={styles.btnPrimary} disabled={busy === 'setup:all'} onClick={() => run({ action: 'setup_check' }, 'setup:all')}>
          run all checks
        </button>
      </div>
      {items.length === 0 ? (
        ...
        <div className={`${styles.grid} ${styles.gridCols}`}>
          {items.map((c) => (
            <article key={c.id} className={styles.card}>
              ...
              <div className={`${styles.mono} ${styles.dim}`} style={{ marginTop: 6 }}>{c.target}</div>
              <div className={styles.cardSummary}>{c.message}</div>
              <div className={styles.actions}>
                <Pill tone={statusTone(c.status)}>{c.status}</Pill>
                <button className={styles.btn} disabled={busy === `setup:${c.id}`} onClick={() => run({ action: 'setup_check', checkId: c.id }, `setup:${c.id}`)}>
                  run check
                </button>
              </div>
            </article>
...
function Commands({ items }: { items: CommandCatalogItem[] }) {
  return (
    <section>
      <SectionHead
