...
// NOTE: The standalone build ships the Finder browser (icon / list / column /
// gallery views, navigation, search, sorting). Rich in-app document rendering
// for PDF / DOCX / XLSX is provided by the separate `@extend/pdf-viewer`,
// `@extend/docx-viewer`, and `@extend/xlsx-viewer` packages. When those are not
// installed, file previews fall back to thumbnails plus this lightweight panel.
function FileSystemDocumentPreviewFallback({
  fileName,
  className,
}: {
  src?: string
  ...
  return (
    <div
      className={cn(
        "flex h-full min-h-0 w-full flex-col items-center justify-center gap-2 rounded-2xl bg-muted/30 p-8 text-center",
        className
      )}
    >
...
const LazyPDFViewer = FileSystemDocumentPreviewFallback
const LazyDocxViewerPreview = FileSystemDocumentPreviewFallback
const LazyXlsxViewerPreview = FileSystemDocumentPreviewFallback
...
function FileVisual({
  ...
  const [pageState, setPageState] = React.useState<{
    path: string
    pageIndex: number
    lazyPageUrls: Record<number, string>
  }>(() => ({
    path: file.path,
    pageIndex: 0,
    lazyPageUrls: {},
  ...
  const activePageState =
    pageState.path === file.path
      ? pageState
      : { path: file.path, pageIndex: 0, lazyPageUrls: {} }
  const { pageIndex, lazyPageUrls } = activePageState
        ...
        <button
          ...
          onClick={(event) => {
            ...
            setPageState((previous) => {
              ...
              return {
                ...base,
                pageIndex: Math.max(0, base.pageIndex - 1),
              }
            })
          }}
        ...
        <button
          ...
          onClick={(event) => {
            ...
            setPageState((previous) => {
              ...
              return {
                ...base,
                pageIndex: Math.min(totalPages - 1, base.pageIndex + 1),
              }
            })
          }}
          onDoubleClick={(event) => event.stopPropagation()}
...
function FileSystemSearchField({
  ...
  const input = (
    <div
      className={cn(
        "relative flex h-7 min-w-0 flex-1 items-center rounded-none border border-input bg-popover text-sm text-foreground shadow-xs/5 transition-shadow outline-none not-dark:bg-clip-padding before:pointer-events-none before:absolute before:inset-0 before:rounded-none not-focus-within:before:shadow-[0_1px_--theme(--color-black/4%)] focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-1 focus-within:ring-offset-background dark:bg-input/32 dark:not-focus-within:before:shadow-[0_-1px_--theme(--color-white/6%)]",
        isInline && "max-w-56"
      )}
    >
      <HugeiconsIcon
  ...
  return (
    ...
    >
      <SelectTrigger
        ...
        aria-label="Sort by"
        title="Sort by"
        className="h-7 min-h-7 w-auto min-w-0 shrink-0 border-0 border-[#554d44] [&_svg]:size-4"
      >
        <SelectValue>
      ...
      <DropdownMenuTrigger
        render={
          <Button
            ...
            aria-label="Filter"
            title="Filter"
            className="relative size-7 border-0 sm:size-7"
          />
        }
      >
          ...
          {visibleEntries.map((entry) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={cn(
                    "max-w-full rounded-sm px-1.5 py-px text-center text-xs font-medium leading-tight break-words",
                    isSelected
            ...
            {entries.slice(start, end).map((entry) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex h-7 shrink-0 items-center gap-2 rounded-md px-2 py-1 text-left text-sm font-medium outline-none focus-visible:ring-2 focus-visible:ring-ring",
                    isSelected
                      ? "bg-primary text-primary-foreground"
                      : isOnTrail
                        ? "bg-accent"
