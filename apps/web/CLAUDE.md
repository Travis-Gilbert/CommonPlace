...
- **No raw CSS in the v2 shell. Use Tailwind v4 register utilities.** Tailwind v4 is installed (`tailwindcss` + `@tailwindcss/postcss` ^4.1.18) and `global.css` maps every `--cr-*` console-register token into `@theme` utilities: `bg-cr-ground`, `bg-cr-surface`, `bg-cr-top`, `text-cr-ink`/`text-cr-ink-2`/`text-cr-ink-3`, `border-cr-hairline`, `bg-cr-tint`, `rounded-cr`/`rounded-cr-sm`/`rounded-cr-lg`, `text-cr-body`. Those utilities ARE the design system (HANDOFF-CONSOLE-REGISTER decision 1). Do NOT write or extend `.module.css` files or hand `<style>` for v2 surfaces. The ~64 legacy `.module.css` files under `src/` are the "700KB of per-surface hand CSS" the register spec names as diagnosis #1; they are being migrated out. When you touch a surface that still has a `.module.css`, convert its styling to register utilities in the `.tsx` and delete the module. The register lint (`scripts/lint-console-register.mjs`) bans arbitrary-value classes (`bg-[#fff]`) and raw palette (`bg-red-500`) so utilities cannot carry off-brand values.
